...
class SugarWidgetSubPanelSMSIcon extends SugarWidgetField
{
...
//add module image
		...
		if (!empty($layout_def['attachment_image_only']) && $layout_def['attachment_image_only'] == true) {
			...
			$ret= '<a onClick="return openDialogWindow(this);" href="index.php?module=' . $module
				. '&action=' . $action
				. '&record=' . $record
...
//if requested, add attachement icon.
		if(!empty($layout_def['image2']) && !empty($layout_def['image2_url_field'])){
			...
			$ret.= (empty($filepath)) ? '' : '<a onClick="return openDialogWindow(this);" href="' . $filepath. '" class="listViewTdLinkS1">' . "$icon_img_html</a>";	
		}
